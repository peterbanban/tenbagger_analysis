...
def score_one(stock: Stock, *, db_conn, cache_dir: Path) -> Row:
    try:
        ...
        entry_ok, _, _ = entry_ok_ma(fq_dates, fq_closes, asof_date)
        ...
        score_total = total_score_framework_v2(
            base=score_base,
...
def main(argv: list[str]) -> int:
    ...
    for s in stocks:
        ...
        score_a = score_model_a(mktcap_yi, pe, pb, nm, roe, debt_ratio, dd)
        score_b = score_model_b(mktcap_yi, pe, pb, nm, roe, ret120, ret250, dd)
        base = (0.5 * score_a + 0.5 * score_b) if entry_ok else score_a
