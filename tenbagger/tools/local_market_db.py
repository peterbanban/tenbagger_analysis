...
# Moved from project root: local_market_db.py
...
from tenbagger.data.http import http_get_json
...
def get_all_a_shares() -> List[Stock]:
    ...
    while True:
        j = http_get_json(EM_CLLIST_URL.format(pn=pn))
        data = j.get("data") or {}
        if total is None:
            try:
...
def fetch_kline(secid: str, *, fqt: int, beg: str, end: str) -> List[Tuple[str, float, float, float, float, float, float]]:
    j = http_get_json(EM_KLINE_URL.format(secid=secid, fqt=fqt, beg=beg, end=end))
    data = j.get("data") or {}
...
def main(argv: List[str]) -> int:
    ...
    sp = sub.add_parser("download-kline", help="download daily kline into sqlite (initial + incremental)")
