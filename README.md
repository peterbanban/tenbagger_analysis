...
- `财务运营质量总分`：现金含金量/营运质量/再融资压力/利息保障的综合（来自年报表格代理）
...
| 现金含金量（CFO≈利润） | 5 | 已覆盖：`cash_quality_score()`（年报经营现金流/净利润代理） | 经营现金流、回款、应收变化 |
| 回报能力（ROIC/ROE趋势） | 5 | 部分：ROE 代理（未做趋势/ROIC） | ROIC/ROE趋势、与同行对比 |
| 资产负债（杠杆与再融资压力） | 5 | 部分：资产负债率 + `refi_pressure_score()`（货币资金/短债/净负债代理）+ `interest_coverage_score()`（利息保障代理） | 净负债、短债、利息保障倍数 |
| 运营质量（应收/存货周转） | 5 | 部分：`ops_quality_score()`（应收/收入、存货/收入及其变化代理） | 周转天数、结构变化 |
